...
class EpisodeSeedWrapper(gym.Wrapper):
    """Assign a deterministic, non-overlapping seed to every episode reset."""

    def __init__(self, env: gym.Env, base_seed: int, env_idx: int, n_envs: int):
        super().__init__(env)
    ...
    def reset(self, *, seed=None, options=None):
        episode_seed = self.base_seed + self.env_idx + self.episode_idx * self.n_envs
        self.episode_idx += 1
...
def _create_single_env(config: SimulationConfig, idx: int) -> gym.Env:
    ...
    # AsyncVectorEnv spawns fresh Python workers that may not inherit inline shell
    # env-var assignments. With CUDA_VISIBLE_DEVICES restricted to one GPU, EGL
    # should use local visible device 0 inside the worker.
    os.environ.setdefault("MUJOCO_EGL_DEVICE_ID", "0")
    # Create base environment
    if config.seed is None:
        env = gym.make(config.env_name, enable_render=True)
    else:
        initial_seed = config.seed + idx
        np.random.seed(initial_seed)
        env = gym.make(config.env_name, enable_render=True, seed=initial_seed)
        env = EpisodeSeedWrapper(env, config.seed, idx, config.n_envs)
...
class Args:
    ...
    seed: Optional[int] = None  # None preserves the historical unseeded protocol.
