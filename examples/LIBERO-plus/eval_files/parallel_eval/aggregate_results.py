...
if overall_results["overall"]["total_count"] == 0:
    print(f"No LIBERO-plus result JSON files found under {log_dir}", file=sys.stderr)
    sys.exit(1)

for category in overall_results:
    if overall_results[category]["total_count"] == 0:
        overall_results[category]["success_rate"] = 0.0
        continue
