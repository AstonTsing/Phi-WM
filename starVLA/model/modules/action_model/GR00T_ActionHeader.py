...
class GR00TMIPActionHead(FlowmatchingActionHead):
    """Two-stage MIP action head, without FDM.

    MIP uses the DiT action network as a denoising/regression map. Both stages
    predict the clean action directly; the second stage refines from a noisy
    action-space point near the target during training and from the first-stage
    ...
    def __init__(self, full_config):
        ...
        config = full_config.framework.action_model
        self.mip_t = float(config.get("mip_t", 0.9))
        if not 0.0 < self.mip_t < 1.0:
            raise ValueError(f"mip_t must be in (0, 1), got {self.mip_t}")
        self.mip_action_stage0_weight = float(config.get("mip_action_stage0_weight", 1.0))
        self.mip_action_stage1_weight = float(config.get("mip_action_stage1_weight", 1.0))
    ...
    def _run_action_step(
        ...
        condition_ref = vl_embs[-1] if isinstance(vl_embs, (list, tuple)) else vl_embs
        device = condition_ref.device
        action_features = self.action_encoder(action_input, timesteps)
        state_features = self.state_encoder(state) if state is not None else None
        ...
        future_tokens = self.future_tokens.weight.unsqueeze(0).expand(condition_ref.shape[0], -1, -1)
        sa_embs = (
            torch.cat((state_features, future_tokens, action_features), dim=1)
            if state_features is not None
            else torch.cat((future_tokens, action_features), dim=1)
        ...
        pred = self.action_decoder(model_output)
        return pred[:, -action_input.shape[1] :]
    ...
    def forward(
        ...
        dtype = actions.dtype
        mip_t = torch.full((batch_size,), self.mip_t, device=device, dtype=dtype)
        mip_t_discretized = (mip_t * self.num_timestep_buckets).long()
        zero_t_discretized = torch.zeros((batch_size,), device=device, dtype=torch.long)

        stage0_actions = torch.zeros_like(actions)
        noise = torch.randn(actions.shape, device=device, dtype=dtype)
        mip_t_expanded = mip_t[:, None, None]
        stage1_actions = actions + (1 - mip_t_expanded) * noise

        pred_action_stage0 = self._run_action_step(
            vl_embs=vl_embs,
            action_input=stage0_actions,
            timesteps=zero_t_discretized,
            state=state,
            encoder_attention_mask=encoder_attention_mask,
        )
        pred_action_stage1 = self._run_action_step(
            vl_embs=vl_embs,
            action_input=stage1_actions,
            timesteps=mip_t_discretized,
            state=state,
            encoder_attention_mask=encoder_attention_mask,
        )

        action_loss0 = F.mse_loss((pred_action_stage0 - actions) / self.mip_t, torch.zeros_like(actions))
        action_loss1 = F.mse_loss(
            (pred_action_stage1 - actions) / (1.0 - self.mip_t),
            torch.zeros_like(actions),
        )
        action_loss = (
            self.mip_action_stage0_weight * action_loss0
            + self.mip_action_stage1_weight * action_loss1
        )
        return {
            "loss": action_loss,
            "action_loss": action_loss,
            "mip_action_loss0": action_loss0,
            "mip_action_loss1": action_loss1,
            "pred_action_stage0": pred_action_stage0,
            "pred_action_stage1": pred_action_stage1,
            "pred_action": pred_action_stage1,
        }
    ...
    def predict_action(
        ...
        condition_ref = vl_embs[-1] if isinstance(vl_embs, (list, tuple)) else vl_embs
        batch_size = condition_ref.shape[0]
        device = condition_ref.device
        actions = torch.zeros(
            size=(batch_size, self.action_horizon, self.action_dim),
        ...
        zero_t = torch.zeros((batch_size,), device=device, dtype=torch.long)
        mip_t_cont = torch.full((batch_size,), self.mip_t, device=device, dtype=condition_ref.dtype)
        mip_t = (mip_t_cont * self.num_timestep_buckets).long()
        ...
        pred_action_stage1 = self._run_action_step(
            vl_embs=vl_embs,
            action_input=pred_action_stage0,
            timesteps=mip_t,
            state=state,
            encoder_attention_mask=encoder_attention_mask,
        )
        return pred_action_stage1
...
def get_action_model(config=None):
    ...
    if action_cfg.get("head_type", "flow") == "gr00t_mip":
        return GR00TMIPActionHead(full_config=config)
