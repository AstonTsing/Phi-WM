...
OBS_IMAGE_HEIGHT=${OBS_IMAGE_HEIGHT:-224}
OBS_IMAGE_WIDTH=${OBS_IMAGE_WIDTH:-224}
POLICY_IMAGE_HISTORY=${POLICY_IMAGE_HISTORY:-1}
...
config_yaml=${config_yaml:-examples/Robocasa_tabletop/train_files/starvla_acteffect_robocasa_gr1.yaml}

data_root_dir=${data_root_dir:-/root/tianyi/LDA-1B/playground/Datasets/nvidia/PhysicalAI-Robotics-GR00T-X-Embodiment-Sim}
data_mix=${data_mix:-fourier_gr1_unified_1000_rollout_fdm}
use_delta_action=${use_delta_action:-false}

action_dim=${action_dim:-29}
state_dim=${state_dim:-58}
...
FDM_DYNAMICS_LOSS_WEIGHT=${FDM_DYNAMICS_LOSS_WEIGHT:-1.0}
FDM_STAGE0_LOSS_WEIGHT=${FDM_STAGE0_LOSS_WEIGHT:-0.05}
FDM_STAGE1_LOSS_WEIGHT=${FDM_STAGE1_LOSS_WEIGHT:-0.05}
FDM_JOINT_RANK_LOSS_WEIGHT=${FDM_JOINT_RANK_LOSS_WEIGHT:-0.005}
FDM_RANK_MARGIN=${FDM_RANK_MARGIN:-0.0}
FDM_RANK_TAU=${FDM_RANK_TAU:-0.01}
...
FDM_CONDITION_NUM_IMAGES=${FDM_CONDITION_NUM_IMAGES:-1}
freeze_module_list=${freeze_module_list:-dino_model}
run_root_dir=${run_root_dir:-/root/tianyi/starVLA/playground/Checkpoints/robocasa}
run_id=${run_id:-robocasa_acteffect}
...
echo "=== StarVLA ActEffect RoboCasa-GR1 ==="
...
echo "OBS_IMAGE_SIZE: ${OBS_IMAGE_HEIGHT}x${OBS_IMAGE_WIDTH}"
echo "POLICY_IMAGE_HISTORY: ${POLICY_IMAGE_HISTORY}"
...
echo "FDM_DYNAMICS_LOSS_WEIGHT: ${FDM_DYNAMICS_LOSS_WEIGHT}"
echo "FDM_STAGE0_LOSS_WEIGHT: ${FDM_STAGE0_LOSS_WEIGHT}"
echo "FDM_STAGE1_LOSS_WEIGHT: ${FDM_STAGE1_LOSS_WEIGHT}"
echo "FDM_JOINT_RANK_LOSS_WEIGHT: ${FDM_JOINT_RANK_LOSS_WEIGHT}"
...
accelerate launch \
  ...
  --framework.fdm.dynamics_loss_weight "${FDM_DYNAMICS_LOSS_WEIGHT}" \
  --framework.fdm.stage0_loss_weight "${FDM_STAGE0_LOSS_WEIGHT}" \
  --framework.fdm.stage1_loss_weight "${FDM_STAGE1_LOSS_WEIGHT}" \
  --framework.fdm.joint_rank_loss_weight "${FDM_JOINT_RANK_LOSS_WEIGHT}" \
  ...
  --datasets.vla_data.default_image_resolution "[3,${OBS_IMAGE_HEIGHT},${OBS_IMAGE_WIDTH}]" \
  --datasets.vla_data.image_size "[${OBS_IMAGE_HEIGHT},${OBS_IMAGE_WIDTH}]" \
  --datasets.vla_data.obs_image_size "[${OBS_IMAGE_HEIGHT},${OBS_IMAGE_WIDTH}]" \
  --datasets.vla_data.policy_image_history "${POLICY_IMAGE_HISTORY}" \
