...
from starVLA.dataloader.gr00t_lerobot.multiview_pack import pack_multiview_sequence
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._curr_v3_data_file_path = None
        self._curr_v3_data_file = None
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        if "future_video" in self.modality_configs:
            simplified_modality_meta["future_video"] = {}
            for future_key in self.modality_configs["future_video"].modality_keys:
                base_key = future_key.replace("future_video.", "")
                if base_key not in simplified_modality_meta["video"]:
                    raise KeyError(f"Future video key {future_key!r} maps to missing video key {base_key!r}")
                simplified_modality_meta["future_video"][base_key] = simplified_modality_meta["video"][base_key]
    ...
    def _pack_sample(self, data: dict) -> dict:
        ...
        if self.data_cfg is not None:
            cfg_image_size = self.data_cfg.get("obs_image_size", self.data_cfg.get("image_size", None))
            if cfg_image_size is not None:
                image_size = (int(cfg_image_size[1]), int(cfg_image_size[0]))
        pack_mode = self.data_cfg.get("pack_multiview", None) if self.data_cfg is not None else None

        video_sequences = [data[video_key] for video_key in self.modality_keys["video"]]
        step_images = [
            Image.fromarray(image).resize(image_size)
            for image in pack_multiview_sequence(video_sequences, mode=pack_mode)
        ]
        vggt_images = [
            Image.fromarray(seq[-1]).resize(image_size)
            for seq in video_sequences
            if len(seq) > 0
        ]

        future_sequences = [
            data[video_key] for video_key in self.modality_keys.get("future_video", [])
        ]
        future_images = [
            Image.fromarray(image).resize(image_size)
            for image in pack_multiview_sequence(future_sequences, mode=pack_mode)
        ]
    ...
    def get_trajectory_data_lerobot_v3(self, trajectory_id: int) -> pd.DataFrame:
        ...
        if self._curr_v3_data_file_path != parquet_path or self._curr_v3_data_file is None:
            self._curr_v3_data_file = pd.read_parquet(parquet_path)
            self._curr_v3_data_file_path = parquet_path

        episode_data = self._curr_v3_data_file.loc[
            self._curr_v3_data_file["episode_index"] == trajectory_id
        ].copy()
    ...
    def get_future_video(
        ...
        """Get future video frames by base index. Shape: [T, H, W, C]."""
        step_indices = self.delta_indices[key] + base_index
        trajectory_index = self.get_trajectory_index(trajectory_id)
        step_indices = np.maximum(step_indices, 0)
        step_indices = np.minimum(step_indices, self.trajectory_lengths[trajectory_index] - 1)
