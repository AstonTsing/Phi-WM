...
def resolve_model_roots(input_path: Path) -> list[Path]:
    ...
    roots = {path.parent for path in input_path.rglob("logs") if path.is_dir()}
    roots.update(
        path.parent.parent
        for path in input_path.rglob("steps_*_pytorch_model.pt.log")
...
def collect_results(root: Path, only_checkpoints: set[str] | None = None) -> dict[str, dict[str, tuple[float, int | None]]]:
    ...
    # DiT4DiT batch_eval_libero.sh writes logs under:
    #   checkpoints/steps_XXXXX_pytorch_model.pt.log/eval_YYYYMMDD_HHMMSS/eval_<suite>_gpuN.log
    # If a checkpoint was evaluated multiple times, later eval directories overwrite earlier
    # parsed suite results. Incomplete eval directories with only server logs are ignored.
    checkpoints_root = root / "checkpoints"
    if checkpoints_root.is_dir():
        for checkpoint_log_dir in sorted(checkpoints_root.glob("steps_*_pytorch_model.pt.log")):
            if not checkpoint_log_dir.is_dir():
                continue

            checkpoint_name = checkpoint_log_dir.name[:-4]
            if only_checkpoints is not None and checkpoint_name not in only_checkpoints:
                continue

            for eval_dir in sorted(checkpoint_log_dir.glob("eval_*")):
                if not eval_dir.is_dir():
                    continue
                for suite in SUITES:
                    for log_path in sorted(eval_dir.glob(f"eval_{suite}_gpu*.log")):
                        parsed = parse_log(log_path)
                        if parsed is None:
                            continue
                        results.setdefault(checkpoint_name, {})[suite] = parsed
