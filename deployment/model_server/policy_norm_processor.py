...
class PolicyNormProcessor:
    ...
    def __init__(self, ckpt_path: str, unnorm_key: Optional[str] = None) -> None:
        ...
        try:
            ...
            logger.warning(
                "Disabling state metadata for %s because saved state stats do not match "
                "the current DataConfig: %s",
                self._ckpt_path,
                e,
            )
            self._state_keys = []
        ...
        if not state_metadata_valid and isinstance(self._transform, ComposedModalityTransform):
            self._transform = ComposedModalityTransform(
                transforms=[
                    transform
                    for transform in self._transform.transforms
                    if not any(str(key).startswith("state.") for key in getattr(transform, "apply_to", []))
                ]
            )
    ...
    def video_keys(self) -> List[str]:
        return list(self._data_config.video_keys)
    ...
    def apply_state(self, raw_state: np.ndarray) -> np.ndarray:
        """Apply the training-time state transform to raw env state."""
        raw_state = np.asarray(raw_state, dtype=np.float32)
        if not self._state_keys:
            return raw_state
        if raw_state.shape[-1] != sum(self._state_key_dims.get(k, 1) for k in self._state_keys):
            raise ValueError(
                f"Expected raw state last dim {sum(self._state_key_dims.get(k, 1) for k in self._state_keys)}, "
                f"got {raw_state.shape[-1]} for shape {raw_state.shape}"
            )
