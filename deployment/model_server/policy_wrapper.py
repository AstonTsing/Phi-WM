...
class PolicyServerWrapper:
    ...
    def __init__(
        ...
        if base_vlm_path is not None:
            config_overrides.setdefault("framework", {}).setdefault("qwenvl", {})[
                "base_vlm"
            ] = str(base_vlm_path)
        if dino_model_path is not None:
            config_overrides.setdefault("framework", {}).setdefault("dinov3", {})[
                "model_path"
            ] = str(dino_model_path)
        ...
        self._uses_state = int(action_model_cfg.get("state_dim", 0) or 0) > 0
        vla_data_cfg = model_cfg.get("datasets", {}).get("vla_data", {})
        self._action_mode = str(
            model_cfg.get("datasets", {}).get("vla_data", {}).get("action_mode", "abs")
        )
        self._action_dim = int(action_model_cfg.get("action_dim", 0))
        self._state_dim = int(action_model_cfg.get("state_dim", 0))
        self._multiview_pack = vla_data_cfg.get("pack_multiview", "none")
        default_history = 4 if self._multiview_pack not in (None, "", "none", "None") else 0
        self._policy_image_history = int(vla_data_cfg.get("policy_image_history", default_history))
        ...
        if unnorm_key is not None or len(self._available_unnorm_keys) == 1:
            ...
            self._video_keys = default_proc.video_keys
            logging.info(
    ...
    def metadata(self) -> Dict[str, Any]:
        ...
        base = {
            ...
            "multiview_pack": self._multiview_pack,
            "policy_image_history": self._policy_image_history,
            "uses_state": self._uses_state,
        ...
        if self._default_unnorm_key is not None:
            ...
            base["uses_state"] = self._uses_state and bool(proc.state_keys)
        return base
    ...
    def predict_action(
        ...
        **kwargs,
        ...
        if any("state" in example for example in examples):
            examples = [dict(example) for example in examples]
            for example in examples:
                if "state" in example:
                    example["state"] = proc.apply_state(np.asarray(example["state"], dtype=np.float32))
