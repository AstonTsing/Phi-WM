...
# ============================================================
CKPT_INPUT=${1:-${CKPT_PATHS_STR:-${CKPT:-$CKPT_DEFAULT}}}
...
EvalEnv() {
    ...
    local EVAL_RUN_ID
    EVAL_RUN_ID=$(basename "${LOG_DIR}")
    local VIDEO_OUT_PATH="${SAVE_ROOT}/videos/${CKPT_NAME}/${EVAL_RUN_ID}/8_denoise_steps_n_action_steps_${N_ACTION_STEPS}_max_episode_steps_${MAX_EPISODE_STEPS}_n_envs_${N_ENVS}_$(echo "${ENV_NAME}" | tr '/' '_')"
    ...
    "${ROBOCASA_PYTHON}" "${STARVLA_ROOT}/examples/Robocasa_tabletop/eval_files/simulation_env.py" \
...
RunSingleCkpt() {
    local CKPT_PATH=$1
    local LOG_DIR="${CKPT_PATH}.log/eval_seed${EVAL_SEED}_$(date +%Y%m%d_%H%M%S)"
    local RUN_STATUS=0
