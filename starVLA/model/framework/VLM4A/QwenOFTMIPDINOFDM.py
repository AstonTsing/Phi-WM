...
"""Standalone OFT-prompted Qwen, MIP, DINO, and FDM framework.
...
from starVLA.model.modules.world_model.DINOFeatureDynamics import DINOFeatureDynamicsPredictor, mip_fdm_loss
...
@FRAMEWORK_REGISTRY.register("QwenOFTMIPDINOFDM")
class Qwen_OFT_MIP_DINO_FDM(baseframework):
    """OFT QwenVL + frozen DINOv3 + MIP action head + DINO dynamics loss."""

    def __init__(self, config: Optional[dict] = None, **kwargs) -> None:
        super().__init__()
        self.config = merge_framework_config(QwenOFTMIPDINOFDMDefaultConfig, config)
        ...
        if self.fdm_enabled:
            if self.dino_model is None:
                raise ValueError("QwenOFTMIPDINOFDM requires DINOv3 to be enabled")
        ...
        if len(token_ids) != 1:
            raise ValueError(
                "QwenOFTMIPDINOFDM expects the action placeholder to be a single token. "
    ...
    def _preprocess_dino_flat_views(self, batch_images: List[List]) -> Tuple[torch.Tensor, List[int]]:
        ...
        if len(set(view_counts)) != 1:
            raise ValueError(f"QwenOFTMIPDINOFDM expects same views per sample, got {view_counts}")
    ...
    def _encode_qwen(self, batch_images, instructions):
        ...
        if input_ids is None:
            raise ValueError("QwenOFTMIPDINOFDM requires input_ids for action-token validation")
    ...
    def _encode_future_dino_tokens_for_fdm(self, future_images, device, dtype):
        if not self.fdm_multi_horizon:
            ...
            if raw is None:
                raise ValueError("QwenOFTMIPDINOFDM requires DINOv3 future tokens")
            return raw, patches
    ...
    def _compute_fdm_loss(self, action_output, current_tokens, target_tokens, patch_indices):
        ...
        if missing:
            raise KeyError(f"QwenOFTMIPDINOFDM requires MIP predictions, missing {missing}")
    ...
    def forward(self, examples: List[dict] = None, **kwargs) -> Tuple:
        ...
        if any(images is None or len(images) == 0 for images in future_images):
            raise KeyError("QwenOFTMIPDINOFDM requires sample['future_image']; use a *_video_fdm data mix")
