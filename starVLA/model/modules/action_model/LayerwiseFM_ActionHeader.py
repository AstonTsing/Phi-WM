...
class LayerwiseMIPActionHead(LayerwiseFlowmatchingActionHead):
    """Layer-wise cross-attention action head with two-stage MIP actions."""

    def __init__(self, global_config, **kwargs):
        super().__init__(global_config=global_config, **kwargs)
        action_config = global_config.framework.action_model
        self.mip_t = float(action_config.get("mip_t", 0.9))
        if not 0.0 < self.mip_t < 1.0:
            raise ValueError(f"mip_t must be in (0, 1), got {self.mip_t}")
        self.mip_action_stage0_weight = float(action_config.get("mip_action_stage0_weight", 1.0))
        self.mip_action_stage1_weight = float(action_config.get("mip_action_stage1_weight", 1.0))
    ...
    def _run_action_step(
        ...
        if len(vl_embs_list) != len(self.model.transformer_blocks):
            raise ValueError(
                f"Expected {len(self.model.transformer_blocks)} layer-wise condition tensors, "
                f"got {len(vl_embs_list)}"
            )
    ...
    def forward(
        ...
        stage0_target_velocity = actions

        noise = torch.randn(actions.shape, device=device, dtype=dtype)
        mip_t_expanded = mip_t[:, None, None]
        stage1_actions = mip_t_expanded * actions + (1 - mip_t_expanded) * noise
        stage1_target_velocity = actions - noise

        pred_velocity_stage0 = self._run_action_step(
        ...
        pred_velocity_stage1 = self._run_action_step(
            vl_embs_list=vl_embs_list,
        ...
        action_loss0 = F.mse_loss(pred_velocity_stage0, stage0_target_velocity)
        action_loss1 = F.mse_loss(pred_velocity_stage1, stage1_target_velocity)
        action_loss = (
        ...
        return {
            ...
            "pred_velocity_stage0": pred_velocity_stage0,
            "pred_velocity_stage1": pred_velocity_stage1,
            "pred_velocity": pred_velocity_stage1,
        }
    ...
    def predict_action(
        ...
        mip_t_cont = torch.full((batch_size,), self.mip_t, device=device, dtype=vl_embs_list[0].dtype)
        ...
        actions = actions + self.mip_t * pred_velocity_stage0
        ...
        actions = actions + (1 - self.mip_t) * pred_velocity_stage1
        return actions
...
def get_action_model(config=None):
    ...
    if action_cfg.get("head_type", "flow") == "layerwise_mip":
        return LayerwiseMIPActionHead(global_config=config)
