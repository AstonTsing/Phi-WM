...
from starVLA.training.trainer_utils.trackers import build_trackers
...
class VLATrainer(TrainerUtils):
    ...
    def _init_trackers(self):
        """Initialize the metric backends named by `cfg.trackers`."""
        self.trackers = build_trackers(self.config, enabled=self.accelerator.is_main_process)
    ...
    def _save_checkpoint(self):
        ...
        state_dict = self.accelerator.get_state_dict(self.model) if zero3 else None
    ...
    def _log_metrics(self, metrics):
        ...
        if self.completed_steps % self.config.trainer.logging_frequency == 0 and dist.get_rank() == 0:
            ...
            self.trackers.log(metrics, self.completed_steps)
    ...
    def _train_step(self, batch_vla, batch_vlm=None):
        ...
        for name, value in output_dict.items():
            if name == "action_loss" or not torch.is_tensor(value) or value.numel() != 1:
                continue
            metrics[name] = value.detach().float().item()
        return metrics
