...
class PolicyWarper:
    def __init__(
        ...
        self.server_metadata = server_meta
        self.uses_state = bool(server_meta.get("uses_state", False))
    ...
    def step(self, observations, **kwargs) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
        ...
        input_state = np.concatenate(input_state, axis=-1).astype(np.float32)
        if self.uses_state and input_state.shape[-1] != 29:
            raise ValueError(f"Expected raw Robocasa state last dim 29 before server transform, got {input_state.shape}")
        ...
        input_state = [input_s for input_s in input_state]  # B, state_history, raw_state_dim
