...
class DiT(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        layerwise_conditions = isinstance(encoder_hidden_states, (list, tuple))
        if layerwise_conditions:
            expected_conditions = sum(
                not (idx % 2 == 1 and self.config.interleave_self_attention)
                for idx in range(len(self.transformer_blocks))
            )
            if len(encoder_hidden_states) != expected_conditions:
                raise ValueError(
                    f"Expected {expected_conditions} layer-wise conditions, "
                    f"got {len(encoder_hidden_states)}"
                )
            encoder_hidden_states = [condition.contiguous() for condition in encoder_hidden_states]
            if isinstance(encoder_attention_mask, (list, tuple)) and len(encoder_attention_mask) != expected_conditions:
                raise ValueError(
                    f"Expected {expected_conditions} layer-wise attention masks, "
                    f"got {len(encoder_attention_mask)}"
                )
        else:
        ...
        for idx, block in enumerate(self.transformer_blocks):
            if idx % 2 == 1 and self.config.interleave_self_attention:
                ...
                condition = encoder_hidden_states[condition_idx] if layerwise_conditions else encoder_hidden_states
                condition_mask = (
                    encoder_attention_mask[condition_idx]
                    if layerwise_conditions and isinstance(encoder_attention_mask, (list, tuple))
